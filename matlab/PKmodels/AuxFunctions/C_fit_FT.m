...
elseif strcmp(func2str(model.irf.model),'TH_Sourbron')
    ...
    Ns=length(t)*3*ones(no_of_curves,1);
...
%     t_cut=t(end)-x_aif(selected_Ns,end)-x_irf(selected_Ns,end);
    ...
    if nargout<2
        % without derivatives
        aif=zeros(length(w),no_of_curves,class(t));
        h=zeros(length(w),no_of_curves,class(t));
        for n=1:no_of_curves
            aif(:,n)=model.aif.model(x_aif(ind_vector(n),:),w,t_cut(n));
